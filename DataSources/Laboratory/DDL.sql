...
GO
ALTER TABLE LAB_EVENTS DROP CONSTRAINT IF EXISTS labevents_fk_itemid;
ALTER TABLE LAB_EVENTS
ADD CONSTRAINT labevents_fk_itemid
    FOREIGN KEY (ITEM_ID)
    REFERENCES D_LAB_ITEMS(ITEM_ID);
GO

--- CREATE INDEX ---
CREATE INDEX D_LABITEMS_idx01
    ON D_LAB_ITEMS (ITEM_ID);

CREATE INDEX D_LABITEMS_idx02
    ON D_LAB_ITEMS (LABEL);

CREATE INDEX D_LABITEMS_idx03
    ON D_LAB_ITEMS (LOINC_CODE);

CREATE INDEX LABEVENTS_idx01
    ON LAB_EVENTS (PATIENT_ID);

CREATE INDEX LABEVENTS_idx02
    ON LAB_EVENTS (ADMISSION_ID);

CREATE INDEX LABEVENTS_idx03
    ON LAB_EVENTS (ITEM_ID);
